<template>
  <v-card>
    ...
    <v-card-text>
      <div class="mb-2">
        <div class="d-flex justify-space-between">
          <span class="caption grey--text">User</span><span class="h6 caption">{{ withdraw.requestor }}</span>
        </div>
        <div class="d-flex justify-space-between">
          <span class="caption grey--text">Email</span><span class="h6 caption">{{ withdraw.email }}</span>
        </div>
        <div class="d-flex justify-space-between">
      ...
      <div class="mb-2">
        ...
        <div class="d-flex justify-space-between">
          <span class="caption grey--text">Local amount</span><span class=" caption">{{ withdraw.localAmount }}
    ...
    <v-card-text v-if="withdraw.status !== 'Refunded' && !revealActions" class="code" v>
      <v-sheet class="d-flex flex-column justify-center pa-3 rounded-sm" color="">
        <div class="caption">Confirmation Code: {{ otp }}</div>
        <v-progress-linear :color="changeColor" rounded v-model="progress"></v-progress-linear>
        <v-text-field color="green" name="name" label="Enter Code" single-line small v-model="iotp"></v-text-field>
      </v-sheet>
    </v-card-text>
